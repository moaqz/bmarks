...
interface Props {
  ...
  isOpen: boolean;
  handleClose: () => void;
  onSubmit: (selectedTag: Tag | null) => Promise<undefined | object>;
}

export function TagSelector(props: Props) {
  const { handleClose, isOpen, defaultValue, onSubmit } = props;
  ...
  return (
    <Modal title="Apply tag" handleClose={handleClose} isOpen={isOpen}>
      <p className="text-sm text-balance text-gray-11 text-center">
        Select a tag to associate with this document. You can also remove the
        existing tag by selecting "None."
      </p>
      ...
      <div className="flex items-center justify-center mt-6">
        {isLoading ? (
          ...
          <Listbox value={selectedTag} onChange={setSelectedTag}>
            <ListboxButton className="bg-transparent border rounded font-medium text-sm focus:outline focus:outline-2 focus:outline-offset-2 w-60 h-10 border-gray-5 focus:outline-gray-8">
              {selectedTag ? selectedTag.name : "Select tag"}
            </ListboxButton>

            <ListboxOptions
              anchor="bottom"
              className="w-60 flex flex-col p-1 rounded bg-gray-2 border border-gray-5 outline-none max-h-60! overflow-y-scroll mt-1"
            >
